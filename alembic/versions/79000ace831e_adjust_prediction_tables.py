...
def upgrade() -> None:
    ...
    op.alter_column("games", "date", existing_type=sa.DATE(), nullable=False)
...
def downgrade() -> None:
    ...
    op.alter_column("games", "date", existing_type=sa.DATE(), nullable=True)
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(
            ["player_id"],
            ["players.player_id"],
            name="player_stats_predictions_player_id_fkey",
        ),
        sa.ForeignKeyConstraint(
            ["prediction_id"],
            ["predictions.prediction_id"],
            name="player_stats_predictions_prediction_id_fkey",
        ),
        sa.PrimaryKeyConstraint(
            "prediction_id", "player_id", name="player_stats_predictions_pkey"
        ),
    )
    op.create_table(
        "player_game_stats",
        ...
        sa.ForeignKeyConstraint(
            ["game_id"], ["games.game_id"], name="player_game_stats_game_id_fkey"
        ),
        sa.ForeignKeyConstraint(
            ["player_id"],
            ["players.player_id"],
    ...
    op.create_table(
        ...
        sa.Column(
            "predicted_winner_team_id", sa.INTEGER(), autoincrement=False, nullable=True
        ),
        sa.Column(
            "timestamp", postgresql.TIMESTAMP(), autoincrement=False, nullable=True
        ),
        sa.ForeignKeyConstraint(
            ["game_id"], ["games.game_id"], name="predictions_game_id_fkey"
        ),
        sa.ForeignKeyConstraint(
            ["predicted_winner_team_id"],
            ["teams.team_id"],
            name="predictions_predicted_winner_team_id_fkey",
        ),
