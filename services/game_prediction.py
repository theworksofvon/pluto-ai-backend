...
class GamePredictionService:
    ...
    def _load_dataset(self) -> pd.DataFrame:
        ...
        try:
            ...
            logger.error(f"Dataset file not found at {DATASET_PATH}. Service will lack historical context.")
    ...
    def _get_team_id_from_name(self, team_name: str) -> Optional[int]:
         ...
         if self.team_data_df.empty:
             ...
             try:
                 ...
                 if team:
                     logger.info(f"Mapped {team_name} to ID {team['id']} via nba_api.stats.static.teams")
                     return team['id']
                 else:
         ...
         if not match.empty:
             ...
             logger.warning(f"Could not find TEAM_ID for {team_name} in the dataset. Trying fallback.")
    ...
    def _get_team_id_from_name_fallback(self, team_name: str) -> Optional[int]:
        ...
        try:
            ...
            team = next((t for t in nba_teams if t['abbreviation'] == team_name), None)
            if team:
                logger.info(f"Mapped {team_name} to ID {team['id']} via nba_api.stats.static.teams (fallback)")
                return team['id']
            else:
                logger.error(f"Could not find team ID for {team_name} via fallback.")
                return None
    ...
    def _get_team_roster(self, team_id: int) -> List[Dict]:
        ...
        try:
            ...
            logger.info(f"Successfully fetched roster with {len(roster_list)} players for team ID: {team_id}")
            return roster_list
        except Exception as e:
            logger.error(f"Error fetching roster for team ID {team_id} from nba_api: {e}")
            return []
    ...
    def _get_game_odds(self, home_team_abbr: str, away_team_abbr: str, game_id: Optional[str]) -> Dict:
        """
        ...
        """
        logger.warning(f"Betting odds data not available via standard nba_api for {home_team_abbr} vs {away_team_abbr}. Returning 'not_available'. Use a dedicated odds API/adapter.")
    ...
    async def prepare_game_prediction_context(
        ...
        prediction_date: Optional[datetime] = None # Date for which prediction is needed
    ) -> Dict[str, Any]:
        """
        ...
        """
        logger.info(f"Preparing game prediction context for {home_team_abbr} vs {away_team_abbr}")
        ...
        if not home_team_id or not away_team_id:
             message = f"Could not resolve Team IDs for {home_team_abbr} or {away_team_abbr}."
        ...
        if not self.team_data_df.empty:
            home_latest_context = self.team_data_df[
                (self.team_data_df['TEAM_ID'] == home_team_id) &
                (self.team_data_df['GAME_DATE'].dt.date < effective_date)
            ].sort_values(by='GAME_DATE', ascending=False).iloc[0:1]

            away_latest_context = self.team_data_df[
                (self.team_data_df['TEAM_ID'] == away_team_id) &
                (self.team_data_df['GAME_DATE'].dt.date < effective_date)
            ].sort_values(by='GAME_DATE', ascending=False).iloc[0:1]
            
            if home_latest_context.empty or away_latest_context.empty:
                logger.warning(f"Insufficient historical data found in dataset for {home_team_abbr} or {away_team_abbr} before {effective_date}. Context will be limited.")
            else:
                ...
                home_performance = self._extract_team_performance(home_context_dict, is_opponent=False)
                away_performance = self._extract_team_performance(away_context_dict, is_opponent=False)
                h2h_context = self._extract_h2h_performance(home_context_dict, int(away_team_id))
                adv_metrics = self._extract_advanced_metrics(home_context_dict, away_context_dict)
                rest_and_b2b = self._extract_rest_and_b2b(home_context_dict, away_context_dict)
                streaks = self._extract_streaks(home_context_dict, away_context_dict)
        else:
             logger.warning("Proceeding without historical data as dataset was not loaded.")
        ...
        vegas_factors = self._get_game_odds(home_team_abbr, away_team_abbr, game_id) # Placeholder
             
        # Calculate Injury Impact using live data (even if placeholder injuries)
        injury_impact = self._calculate_injury_impact(home_injuries, away_injuries, home_roster, away_roster)
    ...
    def _extract_team_performance(self, context_dict: Dict, is_opponent: bool) -> Dict:
         ...
         return {
             ...
             "last_5_avg_plus_minus": context_dict.get(f'{prefix}AVG_PLUS_MINUS_L5', np.nan),
             "last_10_avg_plus_minus": context_dict.get(f'{prefix}AVG_PLUS_MINUS_L10', np.nan),
             "offensive_rating": context_dict.get(f'{prefix}SEASON_AVG_OFF_RTG_APPROX', np.nan),
             "defensive_rating": context_dict.get(f'{prefix}SEASON_AVG_DEF_RTG_APPROX', np.nan),
             "net_rating": context_dict.get(f'{prefix}SEASON_AVG_PLUS_MINUS', np.nan),
         }

    def _extract_h2h_performance(self, home_context_dict: Dict, away_team_id: int) -> Dict:
        ...
        if opponent_in_context is not None and pd.notna(opponent_in_context) and opponent_in_context == float(away_team_id):
             h2h_win_pct = home_context_dict.get('SEASON_H2H_WIN_PCT', np.nan)
        elif opponent_in_context is not None: # Log only if opponent ID was present but didn't match
            logger.warning(f"Latest context opponent ({int(opponent_in_context) if pd.notna(opponent_in_context) else 'N/A'}) doesn't match current opponent ({away_team_id}). H2H may not reflect direct matchup history.")
        
        return {
            "home_team_h2h_win_pct_vs_away": h2h_win_pct,
            "away_team_h2h_win_pct_vs_home": 1.0 - h2h_win_pct if pd.notna(h2h_win_pct) else np.nan
        }
    ...
    def _extract_advanced_metrics(self, home_context: Dict, away_context: Dict) -> Dict:
         ...
         return {
            "home_season_avg_off_rtg": home_context.get('SEASON_AVG_OFF_RTG_APPROX', np.nan),
            "home_season_avg_def_rtg": home_context.get('SEASON_AVG_DEF_RTG_APPROX', np.nan),
            "away_season_avg_off_rtg": away_context.get('SEASON_AVG_OFF_RTG_APPROX', np.nan),
            "away_season_avg_def_rtg": away_context.get('SEASON_AVG_DEF_RTG_APPROX', np.nan),
    ...
    def _calculate_injury_impact(
        ...
        """
        home_impact_score = self._evaluate_single_team_injury_impact(home_injuries, home_roster)
        away_impact_score = self._evaluate_single_team_injury_impact(away_injuries, away_roster)
    ...
    def _evaluate_single_team_injury_impact(self, injuries: List[Dict], roster: List[Dict]) -> float:
        ...
        if roster_size == 0: return 0.0

        # Return fraction of roster injured (capped at 1.0)
        impact = min(num_injured / roster_size, 1.0) 
        logger.debug(f"Calculated injury impact (placeholder): {impact:.2f} ({num_injured}/{roster_size})")
        return impact
    ...
    def _get_game_model_prediction(
        ...
        try:
            ...
            if home_nr is None or away_nr is None or pd.isna(home_nr) or pd.isna(away_nr):
